...
@Injectable()
export class ExchangeRatesApiSource implements RateSource {
  public readonly name = 'Exchange Rates API';
  private readonly logger = new Logger(ExchangeRatesApiSource.name);
  ...
  constructor(
    ...
    if (!this.apiKey) {
      this.logger.warn('Exchange Rates API key not found in configuration');
    }
  }
  ...
  async isAvailable(): Promise<boolean> {
    ...
    try {
      ...
      this.logger.error(`Exchange Rates API is not available: ${error.message}`);
      return false;
    }
  }
  ...
  async getRates(): Promise<RateSourceResponse[]> {
    ...
    try {
      ...
      this.logger.error(`Failed to fetch rates from Exchange Rates API: ${error.message}`);
      throw error;
    }
  }
